...
class formulairemedecin extends StatefulWidget {
  const formulairemedecin({super.key});

  @override
  State<formulairemedecin> createState() => _formulairemedecinState();
}
...
class _formulairemedecinState extends State<formulairemedecin> {
  ...
  Future<String> adddata() async {
    ...
    var result1 = await userReff.add({
      "nom": nom,
      "prenon": prenom,
      "email": email,
    ...
    await listerv(id: result1.id);
    return 'succes';
  }

  listerv({required String id}) async {
    CollectionReference RV = FirebaseFirestore.instance.collection("medecin");

    RV.doc(id).collection('listeRVP').add({});
  }

  RegExp regExp1 =
      new RegExp(r"^[a-zA-Z0-9.a-zA-Z0-9.!#$%&'*+-/=?^_`{|}~]+@mtabibi.com");
...
/////////////////////////////////////////////////////////////////////////////////
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Formulaire de médecin"),
        centerTitle: true,
        leading: IconButton(
